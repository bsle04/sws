...
for m = 1:length(metricFields)
    ...
    % Differences
    overall_diffs = V2_data - V1_data;
    carrier_diff = mean(overall_diffs(:, carrierIdx), 2, 'omitnan');
    noncarrier_diff = mean(overall_diffs(:, nonCarrierIdx), 2, 'omitnan');
...
% Extract years between V1 and V2 for each subject
v1v2diff = demographics.v1v2_diff'; % Ensure it’s a row vector for broadcasting

annualized_diffs = struct();

% Annualized difference plotting
...
for m = 1:length(metricFields)
    ...
    raw_diff = V2_data - V1_data;
    annualized = raw_diff ./ v1v2diff;  % broadcasting across subjects

    % Store annualized differences
    annualized_diffs.(field).all = mean(annualized, 2, 'omitnan');
    annualized_diffs.(field).carrier = mean(annualized(:, carrierIdx), 2, 'omitnan');
    annualized_diffs.(field).noncarrier = mean(annualized(:, nonCarrierIdx), 2, 'omitnan');

    % Plot: 4 rows x 3 cols (all, carrier, non-carrier)
    subplot(4, 3, (m-1)*3 + 1);
    topoplot(annualized_diffs.(field).all, chanlocs);
    title([label ' - All']);
    colorbar;

    subplot(4, 3, (m-1)*3 + 2);
    topoplot(annualized_diffs.(field).carrier, chanlocs);
    title([label ' - Carrier']);
    colorbar;

    subplot(4, 3, (m-1)*3 + 3);
    topoplot(annualized_diffs.(field).noncarrier, chanlocs);
    title([label ' - Non-carrier']);
    colorbar;
end

sgtitle('Annualized Topoplots (V2 - V1) / Years', 'FontSize', 16);

% Save the figure
saveas(gcf, 'C:\Users\Brandon\Repositories\sws\figures\Annualized_Topoplots.png');

% Save the data
save('C:\Users\Brandon\Repositories\sws\data\annualized.mat', 'annualized');
